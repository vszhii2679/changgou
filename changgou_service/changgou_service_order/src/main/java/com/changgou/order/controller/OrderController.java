...
public class OrderController {
    ...
    public Result<Order> add(@RequestBody Order order) {
        //1、从载荷中获取用户名并存入order对象中
        String username = tokenDecode.getUserInfo().get("username");
        order.setUsername(username);
        //2、调用增添订单的方法
        Map map = orderService.add(order);
    ...
    public Result bulkSend(@RequestBody List<Order> orderList) {
        orderService.bulkSend(orderList);
        return new Result(true, StatusCode.OK, "批量发货成功");
    }

    @GetMapping("/confirmTake/{orderId}/operator/{operator}")
    public Result confirmTake(@PathVariable("orderId") String orderId, @PathVariable("operator") String operator) {
        orderService.confirmTake(orderId, operator);
